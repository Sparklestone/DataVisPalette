...
function r2h(r, g, b) { function cl(v) { return Math.max(0,Math.min(255,Math.round(v))); } return "#" + [r,g,b].map(function(v){ return cl(v).toString(16).padStart(2,"0"); }).join(""); }
...
function r2hsl(r, g, b) { r/=255; g/=255; b/=255; var mx=Math.max(r,g,b),mn=Math.min(r,g,b); var hh=0,ss=0,ll=(mx+mn)/2; if(mx!==mn){var d=mx-mn;ss=ll>0.5?d/(2-mx-mn):d/(mx+mn);if(mx===r)hh=((g-b)/d+(g<b?6:0))/6;else if(mx===g)hh=((b-r)/d+2)/6;else hh=((r-g)/d+4)/6;} return {h:hh*360,s:ss*100,l:ll*100}; }
...
function adjustForContrast(hex, bgHex, target) {
  ...
  for (var i = 0; i < 60; i++) { var test = hsl2hex(hsl.h, hsl.s, hsl.l); if (CR(test, bgHex) >= target) return test; hsl.l += isDk ? 1.5 : -1.5; hsl.l = Math.max(5, Math.min(95, hsl.l)); }
...
function generatePalettes(brandColors, optIdx, darkBg) {
  ...
  var cat = baseHues.map(function (base, i) {
    var hue = base.hue, sat = base.sat, swapped = null;
    for (var j = 0; j < brandColors.length; j++) { var bh = hex2hsl(brandColors[j].hex); if (bh.s > 15 && hueDist(hue, bh.h) < 35) { hue = bh.h; sat = bh.s; swapped = brandColors[j].name; break; } }
  ...
  var sem = SEM_BASES.map(function (base, i) { var pair = makePair(base.hue, base.sat, optIdx, darkBg); return { id: i, hue: base.hue, sat: pair.sat, lightHex: pair.lightHex, darkHex: pair.darkHex, label: base.label }; });
  ...
  return (
    <div style={{borderRadius:8,padding:10,backgroundColor:bg,border:"1px solid "+(dark?"rgba(255,255,255,0.08)":"#eee")}}>
      ...
      <div style={{display:"flex",alignItems:"flex-end",gap:2,height:50}}>
        {data.slice(0,slots.length).map(function(v,i){ var hex=useL?slots[i].lightHex:slots[i].darkHex; return (<div key={i} style={{flex:1,height:(v/mx*100)+"%",backgroundColor:hex,borderRadius:"2px 2px 0 0",border:"1px solid "+stroke,boxSizing:"border-box"}} />); })}
      </div>
    </div>
  );
}
...
function Swatch(props) {
  ...
  var rgb=h2r(hex); var cmyk=r2cmyk(rgb.r,rgb.g,rgb.b); var bg=isDark?darkBg:"#ffffff";
  var ratio=CR(hex,bg); var pass=ratio>=4.5; var hov=useState(false);
  return (
...
function ColorDetail(props) {
  ...
  var editH = useState(0); var editS = useState(0); var editL = useState(0); var inited = useRef(false);
  if (!info) return null;
      ...
      <div style={{backgroundColor:"#fff",borderRadius:14,maxWidth:340,width:"100%",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}} onClick={function(e){e.stopPropagation();}}>
        ...
        <div style={{padding:14}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontFamily:"'Space Mono',monospace",fontSize:14,marginBottom:10}}>
            ...
            <div>
              <span style={{color:"#aaa"}}>HSL</span><br />
              <div style={{display:"flex",gap:3,marginTop:2}}>
                <input type="number" value={editH[0]} onChange={function(e){editH[1](parseInt(e.target.value)||0);}} style={{width:44,padding:"2px 4px",border:"1px solid #ddd",borderRadius:4,fontSize:12,fontFamily:"'Space Mono',monospace",textAlign:"center"}} min="0" max="360" />
                <input type="number" value={editS[0]} onChange={function(e){editS[1](parseInt(e.target.value)||0);}} style={{width:38,padding:"2px 4px",border:"1px solid #ddd",borderRadius:4,fontSize:12,fontFamily:"'Space Mono',monospace",textAlign:"center"}} min="0" max="100" />
                <input type="number" value={editL[0]} onChange={function(e){editL[1](parseInt(e.target.value)||0);}} style={{width:38,padding:"2px 4px",border:"1px solid #ddd",borderRadius:4,fontSize:12,fontFamily:"'Space Mono',monospace",textAlign:"center"}} min="0" max="100" />
              </div>
            </div>
...
function BrandStrip(props) {
  var brandColors = props.brandColors;
  if (!brandColors || !brandColors.length) return null;
  return (
    <div style={{backgroundColor:"#fff",borderRadius:10,padding:"10px 14px",border:"1px solid #eee",marginBottom:10}}>
      <span style={{fontSize:11,fontFamily:"'Space Mono',monospace",color:"#aaa",letterSpacing:"0.1em",textTransform:"uppercase",display:"block",marginBottom:6}}>Brand Colors ({brandColors.length})</span>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {brandColors.map(function(c,i){ var rgb=h2r(c.hex); var l=getLum(rgb.r,rgb.g,rgb.b); var fg=l>0.35?"#111":"#fff"; return (
          <div key={i} style={{width:90,borderRadius:8,overflow:"hidden",border:"1px solid #e0e0e0"}}>
            <div style={{height:44,backgroundColor:c.hex,display:"flex",alignItems:"flex-end",padding:"0 6px 3px",boxSizing:"border-box"}}><span style={{fontSize:11,fontWeight:700,color:fg,fontFamily:"'Space Mono',monospace",lineHeight:1}}>{c.name}</span></div>
            <div style={{padding:"4px 6px 5px",backgroundColor:"#fafafa"}}><div style={{fontSize:12,fontWeight:700,color:"#222",fontFamily:"'Space Mono',monospace"}}>{c.hex.toUpperCase()}</div></div>
          </div>
        ); })}
      </div>
    </div>
...
export default function App() {
  ...
  var _compare=useState(false), _brandDD=useState(false);
  var fileRef=useRef(null);
  var brands=_brands[0],setBrands=_brands[1]; var activeBrand=_actBrand[0],setActiveBrand=_actBrand[1];
  var brandColors=_brandColors[0],setBrandColors=_brandColors[1]; var opts=_opts[0],setOpts=_opts[1];
  var activeOpt=_actOpt[0],setActiveOpt=_actOpt[1]; var darkStroke=_darkStroke[0],setDarkStroke=_darkStroke[1];
  var selInfo=_selInfo[0],setSelInfo=_selInfo[1]; var showUpload=_showUpload[0],setShowUpload=_showUpload[1];
  var uploadName=_uploadName[0],setUploadName=_uploadName[1]; var toast=_toast[0],setToast=_toast[1];
  var loaded=_loaded[0],setLoaded=_loaded[1]; var activeTab=_activeTab[0],setActiveTab=_activeTab[1];
  var compare=_compare[0],setCompare=_compare[1]; var brandDD=_brandDD[0],setBrandDD=_brandDD[1];
  function show(msg){ setToast(msg); setTimeout(function(){setToast("");},2500); }
  function regen(bc,ds){ setOpts([generatePalettes(bc,0,ds),generatePalettes(bc,1,ds),generatePalettes(bc,2,ds)]); }

  useEffect(function(){ sGet("dvcs-brands").then(function(b){if(b)setBrands(b);setLoaded(true);}); },[]);
  useEffect(function(){ if(!loaded)return; if(!activeBrand){regen([],darkStroke);setBrandColors([]);} },[loaded,activeBrand]);

  var loadBrand = useCallback(function(key){
    setActiveBrand(key); var b=brands[key]; if(!b) return;
    setBrandColors(b.colors);
    var ds = b.darkStroke || "#032054";
    setDarkStroke(ds);
    regen(b.colors, ds);
  },[brands]);

  function deleteBrand(key){
    var nb=Object.assign({},brands); delete nb[key]; setBrands(nb); sSet("dvcs-brands",nb);
    if(activeBrand===key){setActiveBrand(null);setBrandColors([]);regen([],darkStroke);} show("Deleted");
  }

  function saveDarkStroke(ds) {
    setDarkStroke(ds); regen(brandColors, ds);
    if (activeBrand) { var nb=Object.assign({},brands); if(nb[activeBrand]) nb[activeBrand].darkStroke=ds; setBrands(nb); sSet("dvcs-brands",nb); }
  }
  ...
  var handleUpload = useCallback(function(file){
    ...
    file.text().then(function(text){
      ...
      var hdr=(lines[0]||[]).map(function(h){return h.trim().toLowerCase();}); var hI=hdr.findIndex(function(h){return h==="hex"||h.includes("hex");}); var nI=hdr.findIndex(function(h){return h==="name"||h.includes("name");}); var pI=hdr.findIndex(function(h){return h.includes("pms")||h.includes("pantone");});
      ...
      setDarkStroke(darkest.hex); setActiveBrand(key); setBrandColors(colors); regen(colors,darkest.hex);
      show(uploadName.trim()+" loaded");
    });
  },[brands,uploadName]);

  var hueShift = useCallback(function(oi,type,sid){
    setOpts(function(prev){ var next=JSON.parse(JSON.stringify(prev)); var slots=next[oi][type]; var idx=slots.findIndex(function(s){return s.id===sid;}); if(idx<0)return prev;
    var s=slots[idx]; s.hue=(s.hue+25+Math.floor(Math.random()*45))%360; var pair=makePair(s.hue,s.sat,oi,darkStroke); s.lightHex=pair.lightHex;s.darkHex=pair.darkHex;s.label="H"+Math.round(s.hue)+"\u00B0";s.swapped=null;
    next[oi].spectrum=next[oi].categorical.slice().sort(function(a,b){return a.hue-b.hue;}); return next; });
  },[darkStroke]);

  var lightShift = useCallback(function(oi,type,sid){
    setOpts(function(prev){ var next=JSON.parse(JSON.stringify(prev)); var slots=next[oi][type]; var idx=slots.findIndex(function(s){return s.id===sid;}); if(idx<0)return prev;
    var s=slots[idx]; s.sat=((s.sat+12-30)%60)+30; var pair=makePair(s.hue,s.sat,oi,darkStroke); s.lightHex=pair.lightHex;s.darkHex=pair.darkHex;
    next[oi].spectrum=next[oi].categorical.slice().sort(function(a,b){return a.hue-b.hue;}); return next; });
  },[darkStroke]);
  ...
  var setSlotHSL = useCallback(function(type,sid,newH,newS,newL){
    var oi=activeOpt;
    setOpts(function(prev){ var next=JSON.parse(JSON.stringify(prev)); var slots=next[oi][type]; var idx=slots.findIndex(function(s){return s.id===sid;}); if(idx<0)return prev;
    var s=slots[idx]; s.hue=newH; s.sat=newS; var startHex=hsl2hex(newH,newS,newL);
    s.lightHex=adjustForContrast(startHex,"#ffffff",4.5); s.darkHex=adjustForContrast(startHex,darkStroke,4.5); s.label="H"+Math.round(newH)+"\u00B0"; s.swapped=null;
    next[oi].spectrum=next[oi].categorical.slice().sort(function(a,b){return a.hue-b.hue;}); return next; });
      ...
      <div style={{minHeight:"100vh",background:"#f2f3f6",fontFamily:"'Outfit',sans-serif"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#111,#333)",padding:"18px 24px 14px"}}>
          <div style={{maxWidth:1500,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:33,color:"#fff",letterSpacing:"0.06em"}}>Compare All{activeBrand?" \u00B7 "+brands[activeBrand].name:""}</h1>
            <button onClick={function(){setCompare(false);}} style={{padding:"8px 18px",borderRadius:6,border:"1px solid #555",backgroundColor:"transparent",color:"#ccc",fontSize:15,cursor:"pointer",fontWeight:600}}>Back to Editor</button>
          </div>
        </div>
        <div style={{maxWidth:1500,margin:"0 auto",padding:"16px 16px 50px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
            {opts.map(function(opt,oi){
              var catSlots=opt[activeTab]||opt.categorical; var semSlots=opt.semantic; var deemSlots=opt.deemphasis;
              var lStk="#ffffff"; var dStk=darkStroke;
              return (<div key={oi}>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f2f3f6",fontFamily:"'Outfit',sans-serif"}}>
      <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Outfit:wght@300;400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet" />
      <div style={{background:"linear-gradient(135deg,#111,#333)",padding:"18px 20px 14px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:8}}>
            <div>
              <span style={{fontFamily:"'Space Mono',monospace",fontSize:11,color:"rgba(255,255,255,0.3)",letterSpacing:"0.15em",textTransform:"uppercase"}}>Data Vis Palette</span>
              <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:33,color:"#fff",lineHeight:1,marginTop:2,letterSpacing:"0.05em"}}>{activeBrand?brands[activeBrand].name:"No Brand"}</h1>
            </div>
            <div style={{display:"flex",gap:5}}>
              <button onClick={function(){setShowUpload(true);}} style={{padding:"6px 14px",borderRadius:6,border:"1px solid rgba(255,255,255,0.2)",backgroundColor:"rgba(255,255,255,0.06)",color:"#fff",fontWeight:600,fontSize:13,cursor:"pointer"}}>+ Upload Brand</button>
              <button onClick={function(){setCompare(true);}} style={{padding:"6px 14px",borderRadius:6,border:"1px solid rgba(255,255,255,0.2)",backgroundColor:"rgba(255,255,255,0.06)",color:"#fff",fontWeight:600,fontSize:13,cursor:"pointer"}}>Compare All</button>
            </div>
          </div>
        </div>
      </div>

      <div style={{maxWidth:1200,margin:"10px auto 0",padding:"0 16px"}}><BrandStrip brandColors={brandColors} /></div>

      {/* Dark Stroke Selector */}
      <div style={{maxWidth:1200,margin:"0 auto",padding:"0 16px"}}>
        <div style={{backgroundColor:"#fff",borderRadius:10,padding:"10px 14px",display:"flex",gap:12,alignItems:"center",flexWrap:"wrap",border:"1px solid #eee"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:11,fontFamily:"'Space Mono',monospace",color:"#888",letterSpacing:"0.1em",textTransform:"uppercase"}}>Dark Stroke</span>
            <div style={{position:"relative",width:28,height:28,borderRadius:6,backgroundColor:darkStroke,border:"2px solid #ddd",cursor:"pointer",overflow:"hidden"}}>
              <input type="color" value={darkStroke} onChange={function(e){saveDarkStroke(e.target.value);}} style={{position:"absolute",inset:-4,width:"140%",height:"140%",cursor:"pointer",opacity:0}} />
            </div>
            <span style={{fontSize:11,fontFamily:"'Space Mono',monospace",color:"#aaa"}}>{darkStroke.toUpperCase()}</span>
          </div>
          {brandColors.length>0 && (<div style={{display:"flex",gap:3,alignItems:"center"}}>
            <span style={{fontSize:10,color:"#bbb",fontFamily:"'Space Mono',monospace"}}>Brand:</span>
            {brandColors.map(function(c,i){return (<div key={i} onClick={function(){saveDarkStroke(c.hex);}} style={{width:22,height:22,borderRadius:4,backgroundColor:c.hex,border:darkStroke===c.hex?"2px solid #ff8800":"1px solid #ddd",cursor:"pointer"}} title={c.name+" "+c.hex} />);})}
          </div>)}
          {Object.keys(brands).length>0 && (<div style={{position:"relative",marginLeft:"auto"}}>
            <button onClick={function(){setBrandDD(!brandDD);}} style={{padding:"5px 12px",borderRadius:6,border:"1px solid #ddd",backgroundColor:activeBrand?"#fff5e6":"#fff",color:"#333",fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
            ...
            {brandDD && (<div><div onClick={function(){setBrandDD(false);}} style={{position:"fixed",inset:0,zIndex:39}} />
              <div style={{position:"absolute",top:"100%",right:0,marginTop:4,backgroundColor:"#fff",borderRadius:8,border:"1px solid #ddd",boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:40,minWidth:200,overflow:"hidden"}}>
                ...
                {Object.entries(brands).map(function(entry){var k=entry[0],v=entry[1];var isAct=activeBrand===k;return (<div key={k} style={{display:"flex",alignItems:"center",borderBottom:"1px solid #f5f5f5"}}>
                  <div onClick={function(){loadBrand(k);setBrandDD(false);}} style={{flex:1,padding:"8px 12px",fontSize:13,fontWeight:isAct?700:400,color:isAct?"#333":"#555",cursor:"pointer",backgroundColor:isAct?"#fff5e6":"#fff"}}>{v.name}<span style={{fontSize:11,color:"#bbb",marginLeft:6}}>{v.colors.length} colors</span></div>
                  <button onClick={function(e){e.stopPropagation();deleteBrand(k);}} style={{padding:"4px 10px",border:"none",backgroundColor:"transparent",color:"#ccc",fontSize:18,cursor:"pointer",lineHeight:1}} title="Delete">&times;</button>
                </div>);})}
              </div>
            </div>)}
          </div>)}
      ...
      <div style={{maxWidth:1200,margin:"8px auto 0",padding:"0 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <div style={{display:"flex",gap:2,backgroundColor:"#fff",borderRadius:6,padding:2,border:"1px solid #eee"}}>
          {["Opt 1","Opt 2","Opt 3"].map(function(lbl,i){return (<button key={i} onClick={function(){setActiveOpt(i);}} style={{padding:"5px 12px",borderRadius:4,border:"none",backgroundColor:activeOpt===i?"#333":"transparent",color:activeOpt===i?"#fff":"#888",fontWeight:activeOpt===i?700:400,fontSize:13,cursor:"pointer"}}>{lbl}</button>);})}
        </div>
        <div style={{display:"flex",gap:2,backgroundColor:"#fff",borderRadius:6,padding:2,border:"1px solid #eee"}}>
          {tabs.map(function(t){return (<button key={t.k} onClick={function(){setActiveTab(t.k);}} style={{padding:"5px 12px",borderRadius:4,border:"none",backgroundColor:activeTab===t.k?"#333":"transparent",color:activeTab===t.k?"#fff":"#888",fontWeight:activeTab===t.k?700:400,fontSize:13,cursor:"pointer"}}>{t.l}</button>);})}
        </div>
      </div>
      ...
      <div style={{maxWidth:1200,margin:"12px auto 0",padding:"0 16px 40px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:16}}>
          {[false,true].map(function(isDk){
            var label=isDk?"D Colors":"L Colors";
            var allSlots=[].concat(
              (cur[activeTab]||cur.categorical).map(function(s){return {hex:isDk?s.darkHex:s.lightHex};}),
              cur.semantic.map(function(s){return {hex:isDk?s.darkHex:s.lightHex};}),
              cur.deemphasis.map(function(s){return {hex:isDk?s.darkHex:s.lightHex};})
            );
            return (<div key={isDk?"dt":"lt"}>
              <div style={{fontSize:14,fontWeight:700,fontFamily:"'Space Mono',monospace",color:"#555",marginBottom:6}}>{label}</div>
              <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Space Mono',monospace",fontSize:11}}>
                <thead><tr style={{borderBottom:"2px solid #333"}}><th style={{textAlign:"left",padding:"4px 6px",color:"#999",fontSize:10}}></th><th style={{textAlign:"left",padding:"4px 6px",color:"#999",fontSize:10}}>Hex</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>C</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>M</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>Y</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>K</th></tr></thead>
                <tbody>{allSlots.map(function(s,i){ var rgb=h2r(s.hex); var pms=findPMS(s.hex); var ck=pms||r2cmyk(rgb.r,rgb.g,rgb.b); return (<tr key={i} style={{borderBottom:"1px solid #eee"}}><td style={{padding:"4px 6px"}}><div style={{width:20,height:20,borderRadius:4,backgroundColor:s.hex,border:"1px solid #ddd"}} /></td><td style={{padding:"4px 6px",fontWeight:700,color:"#222"}}>{s.hex.toUpperCase()}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.c}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.m}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.y}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.k}</td></tr>); })}</tbody>
              </table>
            </div>);
          })}
        </div>
      ...
      {showUpload && (<div style={{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backgroundColor:"rgba(0,0,0,0.5)",backdropFilter:"blur(5px)"}} onClick={function(){setShowUpload(false);}}>
        <div style={{backgroundColor:"#fff",borderRadius:12,maxWidth:380,width:"100%",padding:20,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}} onClick={function(e){e.stopPropagation();}}>
          ...
          <p style={{fontSize:14,color:"#888",marginBottom:12}}>CSV/TSV with Name + Hex columns. Darkest color auto-selected as stroke.</p>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={function(){if(uploadName.trim())fileRef.current.click();else show("Enter a name");}} style={{flex:1,padding:9,borderRadius:7,border:"none",backgroundColor:uploadName.trim()?"#222":"#ccc",color:"#fff",fontWeight:700,fontSize:14,cursor:uploadName.trim()?"pointer":"not-allowed"}}>Choose File</button>
            <button onClick={function(){setShowUpload(false);}} style={{padding:"9px 14px",borderRadius:7,border:"1px solid #ddd",backgroundColor:"#fff",color:"#888",fontWeight:600,fontSize:14,cursor:"pointer"}}>Cancel</button>
          </div>
        </div>
      </div>)}
