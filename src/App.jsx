...
function Swatch(props) {
  ...
  var btnStyle={position:"absolute",width:22,height:22,borderRadius:11,backgroundColor:"rgba(0,0,0,0.15)",color:"rgba(255,255,255,0.4)",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,zIndex:2};
  return (
    ...
    <div style={{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backgroundColor:"rgba(0,0,0,0.5)",backdropFilter:"blur(5px)"}} onClick={onClose}>
      <div style={{backgroundColor:"#fff",borderRadius:14,maxWidth:340,width:"100%",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}} onClick={function(e){e.stopPropagation();}}>
        <div style={{height:110,backgroundColor:info.hex,color:fg,position:"relative",display:"flex",alignItems:"flex-end",padding:14}}>
          {info.onHue&&(<button onClick={function(){info.onHue();onClose();}} style={{position:"absolute",top:10,left:10,width:28,height:28,borderRadius:14,backgroundColor:"rgba(0,0,0,0.15)",color:"rgba(255,255,255,0.4)",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,zIndex:2}} title="Shift hue"><svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="8" cy="8" r="5"/><path d="M8 3v-2M13 8h2M8 13v2M3 8h-2" strokeWidth="1.5"/></svg></button>)}
          {info.onLight&&(<button onClick={function(){info.onLight();onClose();}} style={{position:"absolute",top:10,right:10,width:28,height:28,borderRadius:14,backgroundColor:"rgba(0,0,0,0.15)",color:"rgba(255,255,255,0.4)",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,zIndex:2}} title="Cycle tone"><svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M1 1v5h5"/><path d="M15 15v-5h-5"/><path d="M2.3 10a6 6 0 0 0 10.3 1.5L15 10M1 6l2.4-1.5A6 6 0 0 1 13.7 6"/></svg></button>)}
          {canEdit&&(<div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:36,height:36,borderRadius:18,backgroundColor:"rgba(0,0,0,0.12)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",cursor:"pointer",border:"1.5px solid rgba(255,255,255,0.25)"}}>
      ...
      <div style={{maxWidth:1200,margin:"12px auto 0",padding:"0 16px 40px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:16}}>
          {[false,true].map(function(isDk){var label=isDk?"D Colors":"L Colors";var allSlots=[].concat((cur[activeTab]||cur.categorical).map(function(s){return {hex:isDk?s.darkHex:s.lightHex};}),cur.semantic.map(function(s){return {hex:isDk?s.darkHex:s.lightHex};}),cur.deemphasis.map(function(s){return {hex:isDk?s.darkHex:s.lightHex};}));
            return (<div key={isDk?"dt":"lt"}><div style={{fontSize:14,fontWeight:700,fontFamily:"'Space Mono',monospace",color:"#555",marginBottom:6}}>{label}</div><table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Space Mono',monospace",fontSize:11}}><thead><tr style={{borderBottom:"2px solid #333"}}><th style={{textAlign:"left",padding:"4px 6px",color:"#999",fontSize:10}}></th><th style={{textAlign:"left",padding:"4px 6px",color:"#999",fontSize:10}}>Hex</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>C</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>M</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>Y</th><th style={{textAlign:"center",padding:"4px 4px",color:"#999",fontSize:10}}>K</th></tr></thead><tbody>{allSlots.map(function(s,i){var rgb=h2r(s.hex);var pms=findPMS(s.hex);var ck=pms||r2cmyk(rgb.r,rgb.g,rgb.b);return (<tr key={i} style={{borderBottom:"1px solid #eee"}}><td style={{padding:"4px 6px"}}><div style={{width:20,height:20,borderRadius:4,backgroundColor:s.hex,border:"1px solid #ddd"}} /></td><td style={{padding:"4px 6px",fontWeight:700,color:"#222"}}>{s.hex.toUpperCase()}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.c}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.m}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.y}</td><td style={{padding:"4px 4px",textAlign:"center",color:"#555"}}>{ck.k}</td></tr>);})}</tbody></table></div>);
          })}
        </div>
